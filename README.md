...
## General Structure
The KVS syntax will basically list an infinite list of key and value pairs. Each key must be followed by a value, and each value must be followed by a key. The syntax string would end in a value. A value can be one of two types, a **primitive** type or a **structure**. Where the primitive type being a single string of UTF-8 characters.
# Keys
